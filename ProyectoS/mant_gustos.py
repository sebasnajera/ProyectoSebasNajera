...
class MantGustos():
    ...
    def __init__(self, parent):
        ...
        self.lb_gustos = Label(self.raiz, text = "Código:")
        self.lb_gustos.place(x = 100, y = 60)
        self.txt_gustos = Entry(self.raiz, textvariable=self.gustos.gustos, justify="right")
        self.txt_gustos.place(x = 230, y = 60)

        #coloca en el formulario el campo y el label de nombre
        self.lb_descripcion = Label(self.raiz, text = "Gustos y Descripcion:")
        self.lb_descripcion.place(x = 100, y = 90)
        self.txt_descripcion = Entry(self.raiz, textvariable=self.gustos.descripcion, justify="right", width=30)
        self.txt_descripcion.place(x = 230, y = 90)

        self.lb_fk_cedula = Label(self.raiz, text = "Cedula:")
        self.lb_fk_cedula.place(x = 100, y = 120)
        self.txt_fk_cedula = Entry(self.raiz, textvariable=self.gustos.fk_cedula, justify="right", width=30)
        ...
        self.sheet = Sheet(self.raiz,
                           ...
                           #header_align = "w",
                            ...
                            headers = ['Gustos', 'Descripcion', 'Cedula'],
    ...
    def cargarInformacion(self):
        try:
            ...
            gustos = (self.sheet.get_cell_data(datoSeleccionado[0],0))
            self.gustos.gustos.set(gustos)
            self.gustosBo = GustosBO.GustosBO() #se crea un objeto de logica de negocio
    ...
    def eliminarInformacion(self):
        try:
            datoSeleccionado = self.sheet.get_currently_selected()
            gustos = (self.sheet.get_cell_data(datoSeleccionado[0],0))
            descripcion = (self.sheet.get_cell_data(datoSeleccionado[0],1))

            resultado = msg.askquestion("Eliminar",  "¿Desear eliminar a "+descripcion+" de la base de datos?")
            if resultado == "yes":
                self.gustos.gustos.set(gustos)
                self.gustosBo = GustosBO.GustosBO() #se crea un objeto de logica de negocio
    ...
    def enviarInformacion(self):
        try:
            ...
            self.cargarTodaInformacion()
            self.gustos.limpiar() #se limpia el formulario
    ...
    def seleccionarFecha(self):
        self.gustos.fecNacimiento.set(self.cal.selection_get())
